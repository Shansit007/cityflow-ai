 ...
 *
 * Records what the person decided about today's recommendation:
 ...
 * A recommendation nobody acts on tells the system nothing. From Phase 3, these
 * confirmed decisions are what feed aggregated city demand — which is how
 * CityFlow AI notices that a suggested slot is filling up and re-spreads people
 * instead of creating a new peak.
 *
...
export async function POST(request: Request) {
  ...
  const { decision, chosenDeparture } = parsed.data;
  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json(
        { error: "There is no recommendation for today yet." },
        { status: 404 }
      );
    }
    ...
    const updated = await prisma.recommendation.update({
      ...
      data: { status: decision, chosenDeparture: resolvedDeparture },
    });
